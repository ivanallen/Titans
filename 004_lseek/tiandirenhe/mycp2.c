...
int main(int argc, char *argv[]) {
    ...
    off_t srcfilesize = -1;//源文件大小，字节数
    off_t dstfilesize = -1;//目标文件代销，字节数
    ...
    srcfilesize = lseek(srcfd, 0, SEEK_END);//获取源文件大小
    if (-1 == srcfilesize) {
        close(srcfd);
    ...
    currpos = lseek(srcfd, srcfilesize/2,  SEEK_SET);//重新定位文件指针的偏移位置，文件指针指向文件的一半的位置。
    if (-1 == currpos) {
        close(srcfd);
    ...
    if (count == 0) {
        dstfilesize = lseek(dstfd, 0, SEEK_END);//获取目标文件大小
        if (-1 == dstfilesize) {
            close(dstfd);
            close(srcfd);
